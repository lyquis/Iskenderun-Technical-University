...
// break komutu son haneden sonra virgül yazdırmamak için.
    if (a[0] >= b[0] && a[1] <= b[1] ){for ( int i = a[0]; i <= a[1] ;i++){cout << i ; if ( i == a[1] ) {break;} ; cout << ",";}}
    if (a[0] <= b[0] && a[1] <= b[1] ){for ( int i = b[0]; i <= a[1] ;i++){cout << i ; if ( i == a[1] ) {break;} ; cout << ",";}}
    if (a[0] >= b[0] && a[1] >= b[1] ){for ( int i = a[0]; i <= b[1] ;i++){cout << i ; if ( i == b[1] ) {break;} ; cout << ",";}}
    if (a[0] <= b[0] && a[1] >= b[1] ){for ( int i = b[0]; i <= b[1] ;i++){cout << i ; if ( i == b[1] ) {break;} ; cout << ",";}}
    cout << ")";
return 0;
}
